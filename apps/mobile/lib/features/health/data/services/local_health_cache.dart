...
///
/// 오프라인 상태에서 Health 데이터를 로컬에 임시 저장
...
class LocalHealthCache {
  ...
  Future<void> savePending(HealthDaily data) async {
    ...
    await db.insert(
      ...
      {
        'id': key,
        'payload': jsonEncode(data.toSupabase()),
        'attempts': 0,
  ...
  Future<List<HealthDaily>> getPending() async {
    ...
    return rows.map((row) {
      ...
      return HealthDailySupabaseMapper.fromSupabase(payload);
    }).toList();
  }
  ...
  Future<void> removePending(String userId, DateTime date) async {
    final db = await database;
    final key = '${userId}_${date.toIso8601String().substring(0, 10)}';
  ...
  Future<void> incrementAttempts(String userId, DateTime date) async {
    final db = await database;
    final key = '${userId}_${date.toIso8601String().substring(0, 10)}';
    await db.rawUpdate(
  ...
  Future<int> getAttempts(String userId, DateTime date) async {
    final db = await database;
    final key = '${userId}_${date.toIso8601String().substring(0, 10)}';
